...
| M33 | **Störungs-Dauer als Datenfeld statt Konstante** (reines Refactoring, keine fachliche Änderung): `DAUER_TROCKENHEIT_JAHRE` und `DAUER_BORKENKAEFER_MAX_JAHRE` wandern aus `scripts/simulation/model.py` nach `data/stoerungen.json` (`dauer_jahre` je Störung, `null` = ab Trigger dauerhaft), `model.py` liest sie von dort. **Fertig, wenn:** nach erneutem `python3 scripts/simulation/build_simulationen.py` **`git diff data/generated` leer** ist (Null-Diff-Nachweis) | ⬜ offen | Backlog „Mehrjährige Störungsdauer" (Voraussetzung) + zahlt auf „Review der Lehrkraft-Annahmen" ein |
