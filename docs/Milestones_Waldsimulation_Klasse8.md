...
- **M24-Farbpalette nicht im Browser gegengeprüft:** Die neuen `INDIKATOR_STIL`-Farbwerte in `js/chart.js` (14 Kategorien, 2 davon als Zwei-Farben-Familie) wurden von Hand nach Farbton/Helligkeit verteilt, aber mangels Browser-/Screenshot-Werkzeug in der Umsetzungssitzung nicht visuell gegengeprüft. Vor dem Unterrichtseinsatz einmal im Analyse-Screen mehrere Indikatoren gleichzeitig auswählen (v. a. die Borkenkäfer-Kaskade: `fichte_vitalitaet` + `borkenkaefer_dichte` + `totholzmenge` + `blaeuepilz_indikator` + `buntspecht_indikator` + `ameisenbuntkaefer_indikator`) und prüfen, ob alle Farben klar unterscheidbar bleiben; bei Bedarf einzelne Hex-Werte in `INDIKATOR_STIL` nachjustieren.
