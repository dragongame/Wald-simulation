...
- **Teilweise umgesetzt (2026-08-26):** Kurven-Analyse-Aspekt ist jetzt erledigt – 20 neue Indikatoren (die o.g. Arten + Fuchs + ein neuer, erfundener Kleinsäuger-Hilfsindikator) sind in `scripts/simulation/model.py`/`data/indikatoren.json` simuliert und im Analyse-Screen frei wählbar (siehe Milestones-Dokument). **Weiterhin offen:** eigene Dashboard-Kacheln/Sprites für diese Arten im Live-Dashboard (war explizit nicht Teil dieser Runde) sowie eine echte Luchs/Wolf-Simulation als Ersatz für den alten Wildverbiss-Regler (siehe eigener, separat laufender Umsetzungsschritt).
